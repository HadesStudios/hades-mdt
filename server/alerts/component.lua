...
AddEventHandler("Proxy:Shared:RegisterReady", function()
	exports["hades-base"]:RegisterComponent("EmergencyAlerts", _pdAlerts)
end)
...
_pdAlerts = {
	...
	-- Alert assignments: map alertId -> {callsign, ...}
	AssignToAlert = function(self, alertId, callsign)
		if not alertId then return end
		alertAssignments[alertId] = alertAssignments[alertId] or {}
		-- avoid duplicates
		for _, c in ipairs(alertAssignments[alertId]) do
			if c == callsign then
				-- already assigned
				self:SendAlertAssignments()
				return true
			end
		end
		table.insert(alertAssignments[alertId], callsign)
		self:SendAlertAssignments()
		return true
	end,
	RemoveFromAlert = function(self, alertId, callsign)
		if not alertId or not alertAssignments[alertId] then return false end
		for k, c in ipairs(alertAssignments[alertId]) do
			if c == callsign then
				table.remove(alertAssignments[alertId], k)
				-- cleanup empty
				if #alertAssignments[alertId] == 0 then
					alertAssignments[alertId] = nil
				end
				self:SendAlertAssignments()
				return true
			end
	...
	SendAlertAssignments = function(self)
		-- broadcast current assignments to all clients
		for k, v in pairs(emergencyAlertsData) do
			TriggerClientEvent("EmergencyAlerts:Client:UpdateAlertAssignments", k, alertAssignments)
		end
	end,
