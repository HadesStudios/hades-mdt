...
RegisterNetEvent("EmergencyAlerts:Server:AssignToAlert", function(data)
	-- data: { alertId = <id>, callsign = <callsign> }
	if not data then return end
	EmergencyAlerts:AssignToAlert(data.alertId, data.callsign)
end)

RegisterNetEvent("EmergencyAlerts:Server:RemoveFromAlert", function(data)
	if not data then return end
	EmergencyAlerts:RemoveFromAlert(data.alertId, data.callsign)
end)

RegisterNetEvent("EmergencyAlerts:Server:RemoveFromAllAlertsForCallsign", function(callsign)
	if not callsign then return end
	-- remove callsign from all alertAssignments
	for alertId, list in pairs(alertAssignments) do
		for i = #list, 1, -1 do
			if list[i] == callsign then
				table.remove(list, i)
			end
		end
	...
	EmergencyAlerts:SendAlertAssignments()
end)
