					...
					Database.Game:find({
						...
						if char.Jobs then
							for k, v in ipairs(char.Jobs) do
								...
								-- Check if they are the actual owner
								if jobData.Owner and jobData.Owner == char.SID then
									isOwner = true
								end
								
								-- Check if they have the highest grade in the company
								if not isOwner and v.Grade and v.Grade.Level then
									...
									if v.Grade.Level >= highestGrade then
										isOwner = true
									end
								end
