...
const useStyles = makeStyles((theme) => ({
	wrapper: {
		...
		// Hide any radio/connect UI that may be injected by other components
		// (svg[data-icon="radio"] targets FontAwesome radio icon svgs,
		// aria-label*="radio" targets connect buttons, and .radioChip targets
		// any chip-styled radio controls). This removes the 'radio' UI from
		// alerts without touching other components.
		'& svg[data-icon="radio"]': {
						...
						{alert.location ? (
							<div className={classes.minor}>
								<span
									...
									onClick={onRoute}
									onKeyDown={(e) => {
										if (e.key === 'Enter' || e.key === ' ') onRoute();
									}}
									style={{ cursor: 'pointer', display: 'inline-flex', alignItems: 'center' }}
									title="Set GPS to alert location"
								>
									<FontAwesomeIcon className={classes.icon} icon={['fas', 'location']} />
									<span style={{ marginLeft: 6 }}>
										{alert.location.street1} {alert.location.street2 && `/ ${alert.location.street2} `}| {alert.location.area}
									</span>
								</span>
							</div>
						) : (
