...
export default ({ match }) => {
	...
	const onEvidenceLocker = async () => {
		// First look for an evidence item of type 'evidence_locker' on the report
		try {
			const lockerItem = (report.evidence || []).find((e) => e.type === 'evidence_locker');
			let lockerId = null;
			if (lockerItem) lockerId = lockerItem.value || lockerItem.label || lockerItem._id || null;
			if (!lockerId) {
				// fallback: prompt user for locker id via simple prompt NUI (MDT has no built-in Input helper here)
				const promptRes = await (await Nui.send('Prompt', { title: 'Open Evidence Locker', label: 'Locker ID' })).json();
				if (promptRes && promptRes.value) lockerId = promptRes.value;
			}
			if (!lockerId) return toast.error('No Locker ID provided');
			let res = await (await Nui.send('EvidenceLocker', lockerId)).json();
